  ...
  const renderResult = ({ item }: { item: SearchResult }) => (
    <TouchableOpacity style={styles.resultItem}>
      <View style={styles.resultIcon}>
        <Ionicons
          name={item.type === 'user' ? 'person' : item.type === 'hashtag' ? 'pricetag' : 'document-text'}
          size={24}
          color="#666"
        />
      </View>
...
const styles = StyleSheet.create({
  ...
  resultItem: { flexDirection: 'row', alignItems: 'center', padding: 16, backgroundColor: '#ffffff', borderBottomWidth: 1, borderBottomColor: '#f0f0f0' },
  resultIcon: { width: 44, height: 44, borderRadius: 22, backgroundColor: '#f5f5f5', justifyContent: 'center', alignItems: 'center' },
